...
/*!
    ...
    The logic is silly: it determines how many valid values are located within
   the focal area. If this count is an even number, some magic value is
   returned, and if it is odd, some other magic value is returned.
*/
template <typename InputElement> class FocalEvenOdd {
...
public:
  ...
  template <typename Kernel, typename OutputPolicies, typename InputPolicies,
            typename Subspan>
  OutputElement
  operator()(Kernel const &kernel, OutputPolicies const &output_policies,
             InputPolicies const &input_policies, Subspan const &window) const {
...
lue::PartitionedArray<Element, rank> focal_even_odd(Array const &array,
                                                    Kernel const &kernel) {
...
int main([[maybe_unused]] int const argc, [[maybe_unused]] char **argv) {
  ...
  try {
    ...
    Array input_array =
        uniform<Element>(array_shape, partition_shape, min_value, max_value);

    // Introduce some no-data by excluding a range of values [40, 60]
    input_array = where(input_array < Element{40} && input_array > Element{60},
                        input_array);
